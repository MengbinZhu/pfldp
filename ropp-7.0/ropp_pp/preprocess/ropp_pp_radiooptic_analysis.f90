...
!-------------------------------------------------------------------------------
  ...
  ! 2.2 Calculation of coordinates in occultation plane

  CALL plane_coordinates(r_leo,r_gns,r_coc,roc, xleo, yleo, xgns, ygns, ax, ay)
  yleo(:)  = yleo(:) - roc
  ygns(:)  = ygns(:) - roc

  ! 2.3. Calculation of regression coefficients
...
!-------------------------------------------------------------------------------
! 3. Calculation of complex field
!-------------------------------------------------------------------------------
  ...
  ! 3.2 Calculation of accumulated phase
  ...
  ! 3.3 Calculation of grid limits
  ...
  ! 3.5 Calculation of phase model   
...
!-------------------------------------------------------------------------------
! 4. Radiooptical analysis
!-------------------------------------------------------------------------------
  
  DO ic=1,nc

     ! 4.1 Calculation of internal and external scales
        ...
        IF (OutRO) THEN
           IF (PRESENT(filnam)) THEN
              IF (TRIM(filnam) == "ropp_pp_spectra") THEN ! Default in ropp_pp_spectra_tool
                IF (ic == 1) OPEN(UNIT=11,FILE="ROanalysis_ep_L1.dat")
                IF (ic == 2) OPEN(UNIT=11,FILE="ROanalysis_ep_L2.dat")
              ELSE 
                IF (ic == 1) OPEN(UNIT=11,FILE=TRIM(filnam)//"_ep_L1.dat")
                IF (ic == 2) OPEN(UNIT=11,FILE=TRIM(filnam)//"_ep_L2.dat")
              ENDIF
           ELSE ! Use original default output filenames
                IF (ic == 1) OPEN(UNIT=11,FILE="ROanalysis_ep_L1.dat")
                IF (ic == 2) OPEN(UNIT=11,FILE="ROanalysis_ep_L2.dat")
           ENDIF
           WRITE(11, '(A / A,I5,A,I5,A)')                               &
                     'VARIABLES = "bangle (rad)", "impact (km)", "ln|U|"',&
                     'ZONE I=', nr, ' J=', ny, ' F=POINT'
        ENDIF
     ENDIF
          
     ! 4.2 Array allocation
     ...
     ! 4.3 Radiooptical analysis in apertures

     DO i=1,ny

        ! 4.3.1 Calculation of aperture limits
        ...
        ! 4.3.2 Interpolation of phase and amplitude
        ...
        ! 4.3.3 Phase regression

        CALL ropp_pp_init_polynomial(TR, BP)
        CALL ropp_pp_regression(BP, SMR, CP)
        CP(:) = k(ic)*CP(:)

        ! 4.3.4 Downconversion of frequency and calculation of complex signal
        ...
        ! 4.3.5 Calculation of frequencies
        ...
        ! 4.3.6 Calculation of bending angles
        ...
        ! 4.3.8 Computation of weighted values
        ...
        IF (PRESENT(OutRO)) THEN
           IF (OutRO) THEN
!              WRITE(11, '(E13.5,1X,F10.4,1X,E13.5)') &
              WRITE(11, '(E20.10,1X,E20.10,1X,E20.10)') &
                   (EK(i,j), (PK(i,j)-roc)/1000.0_wp, UA(i,j), j=0,nr-1)
           ENDIF
        ENDIF

        ! 4.3.9 Writing spectral maxima
   ...
   ! 4.4 Output argument computation
...
!-------------------------------------------------------------------------------
! 6. Calculate extremum of square polynomial
!-------------------------------------------------------------------------------
  ...
  SUBROUTINE extremum(x, y, xext, yext)
    ...
    ! 6.2 Calculation of interpolation coefficients
