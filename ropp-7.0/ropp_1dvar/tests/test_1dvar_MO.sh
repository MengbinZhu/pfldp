#!/bin/sh
#
#****s* 1d_var/test_1dvar_MO.sh
#
# NAME
#    test_1dvar_MO.sh - Test the ropp_1dvar program.
#
# SYNOPSIS
#   test_1dvar_MO.sh
#
...
#   Produces output netCDF file IT-1DVAR-04.1.nc
#   The 1D-VAR retrieves temperature, water vapor, and pressure from
#   5 COSMIC refractivity and bending angle profiles and co-located Met Office
#   background fields.
...
OBS="../data/IT-1DVAR-04_y.nc"              #observations data
BACK="../data/IT-1DVAR-04_b.nc"             #background data
COV="../data/IT-1DVAR-04_c.nc"              #correlations for bg errors

OUT="../data/IT-1DVAR-"${file_id}".1.nc"        #output solution data
CONFIG="../config/metoffice_bangle_1dvar.cf"    #configuration file

## 1.1 Run executable

if [ -f ../tools/ropp_1dvar_bangle ]; then
  echo "Running ropp_1dvar executable..."${extra_command}
##  ../tools/ropp_1dvar_bangle -y  $OBS -b $BACK --bg-corr $COV -o $OUT -c $CONFIG
...
## 1.4 Show results
  ...
  # Check for output viewer device
    ...
    if [[ $device = " " ]] ; then
      echo "Could not find output software to output plot - check test_1dvar_MO.sh script for options. "
    else
...
BACK="../data/IT-1DVAR-04_b.nc"             #background data
COV="../data/IT-1DVAR-04_c.nc"              #correlations for bg errors
COB="../data/IT-1DVAR-04_o.nc"              #correlations for ob errors

OUT="../data/IT-1DVAR-"${file_id}".1.nc"        #output solution data
CONFIG="../config/metoffice_refrac_1dvar.cf"    #configuration file

## 2.1 Run executable

if [ -f ../tools/ropp_1dvar_refrac ]; then
  echo "Running ropp_1dvar executable..."
##  ../tools/ropp_1dvar_refrac -y  $OBS -b $BACK --bg-corr $COV --obs-corr $COB -o $OUT -c $CONFIG
