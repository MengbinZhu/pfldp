...
;	  28 Aug 92, FKK
;;      added pos_par, allows to give position parameter for plot
;;      command, otherwise legend does not work properly with plots
;;      created by position keyword. 02/03/03 AvE
;-
pro klegend_d,help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
...
if (n_elements(pos_par) gt 0) then begin
    plot,/nodata,xstyle=4,ystyle=4,[0],/noerase, position=pos_par
endif
...
for i = 0,n-1 do begin
  ...
  if vectorfont(i) ne '' then begin
    ...
    if symline and (linestyle(i) ge 0) then plots,xp*number,yp,color=colors(i) $
  ...
  xyouts,x*number,y,items(i),width=width,color=textcolors(i),size=charsize,font=font
  x = x + width
