! $Id: ropp_qc_bgqc.f90 3551 2013-02-25 09:51:28Z idculv $
...
! ------------------------
  ...
  n_good =  COUNT(obs%bangle > -9.0_wp .AND. obs%weights > 0.0_wp)
  idx    => WHERE(obs%bangle > -9.0_wp .AND. obs%weights > 0.0_wp .AND. &
  ...
  IF (config % bgqc_apply) THEN
     ...
     ELSE
        ...
        CALL message(msg_error, &
             ...
             "   The entire profile is flaggd as ""not ok"".\n")
     ENDIF
  ENDIF
...
! ------------------------
  ...
  n_good =  COUNT(obs%refrac > -900.0_wp .AND. obs%weights > 0.0_wp)
  idx    => WHERE(obs%refrac > -900.0_wp .AND. obs%weights > 0.0_wp .AND. &
              ABS(diag%OmB) > MIN(config%bgqc_reject_factor * diag%OmB_sigma, &
              config%bgqc_reject_factor), n_bad)
