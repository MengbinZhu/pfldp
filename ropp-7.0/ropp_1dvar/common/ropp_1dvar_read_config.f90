! $Id: ropp_1dvar_read_config.f90 3551 2013-02-25 09:51:28Z idculv $
...
!----------------------------------------------------------------------------
! 18. Read and interpret 1dVar config text file
!----------------------------------------------------------------------------
...
! 18.1 Read 1dVar config file, ignoring comment lines
    CALL config_readfile(file, lines, comments = '#!;*')
  
! 18.2 Split data lines into keys and values
    nlines = SIZE(lines)
...
!----------------------------------------------------------------------------
! 19. Read 1dVar config text file, excluding empty lines and comments
!----------------------------------------------------------------------------
...
! 19.2 Loop over all lines
    ...
    DO
     ...
     IF (i <= chunksize) THEN
        ...
        i = i + 1 ; j = j + 1
     ELSE
        CALL reallocate(line_arr, SIZE(line_arr) + chunksize)
        line_arr(j) = line
        i = 2 ; j = j + 1
     END IF
...
! 19.3 Close data file
  
  CLOSE(unit)
  
! 19.4 Check for comments
...
! 19.5 Copy found lines to output array
...
!----------------------------------------------------------------------------
! 20. Split strings into two sub-strings, separated by '='
!----------------------------------------------------------------------------
