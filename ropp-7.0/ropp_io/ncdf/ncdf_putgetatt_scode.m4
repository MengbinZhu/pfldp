...
dnl
subroutine NCDF_SFUNATT (attname, value, varname, ncfile, ncid)

  use typeSizes
  use ncdf, not_this => NCDF_SFUNATT

  implicit none

  character(len = *),        intent(in ) :: attname
  TYPE, &
                             intent(IN_OR_OUT`') :: value
...
! -----------------
  ...
  status = `nf90_'PUTORGET`_att(ncid_local, varid, aname, value)'
  if (status /= nf90_noerr) call ncdf_error_handler(status)

end subroutine NCDF_SFUNATT
