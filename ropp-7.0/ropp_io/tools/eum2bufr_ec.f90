...
! Local variables
  ...
  CHARACTER (LEN=256),  &
          DIMENSION(:), &
          ALLOCATABLE :: EUMdsn         ! Name(s) of input EUM file(s)
  ...
  CHARACTER(LEN=20)   :: centre = 'EUM' ! Centre (EUM here)
  CHARACTER(LEN=20)   :: resolution     ! Resolution group of netCDF4 EUM file
...
!--------------------------------------------------------------
  ...
  CALL message_set_routine ( "eum2bufr" )
...
!--------------------------------------------------------------
  ...
  nFiles = MAX ( IARGC(), 1 )
  ALLOCATE ( EUMdsn(nFiles) )
  CALL GetOptions ( EUMdsn,     &
                    nFiles,      &
                    BUFRdsn,     &
                    CSNdsn,      &
                    Thindsn,     &
...
!--------------------------------------------------------------
  ...
  DO ifile = 1, nFiles
    ...
    INQUIRE ( FILE=EUMdsn(ifile), EXIST=exists )
    IF ( .NOT. exists ) THEN
      CALL message ( msg_error, "EUM input file  "// &
                                TRIM(EUMdsn(ifile))//" not found" )
      CYCLE
    ENDIF

    CALL message ( msg_info, "Reading EUM data from "// &
                             TRIM(EUMdsn(ifile)) )

    nprofs = ropp_io_nrec ( EUMdsn(ifile) )
...
!--------------------------------------------------------------
  ...
  IF ( ALLOCATED(EUMdsn) ) DEALLOCATE (EUMdsn)
!!!  CALL EXIT(ErrOK)
...
SUBROUTINE version_info()
  ...
  PRINT *, 'eum2bufr -  EUM netCDF4 to BUFR encoder [ECMWF library]'
  PRINT *, ''
