...
!   'Version' is the version number of this complete application program
  ...
  CHARACTER (LEN=*), PARAMETER :: Version = "V5.1 25-Aug-2011"
...
! Local variables
  ...
  CHARACTER (LEN=20)  :: ocentre = "EUM"
  CHARACTER (LEN=80)  :: outstr                       ! Formatted output string
  CHARACTER (LEN=10)  :: number                       ! Number as string
  CHARACTER (LEN=256) :: resolution = "thinned"       ! Resolution of EUM L1B file to use, default thinned
  LOGICAL             :: getlevel1a = .FALSE.         ! Get also level 1A in ROPP netCDF file
  LOGICAL             :: getbufr = .FALSE.            ! Get all bufr data (level 1a with dimension 1!)
...
!-------------------------------------------------------------
  ...
  CALL message_set_routine ( "eum2ropp" )
...
!-------------------------------------------------------------
  ...
  DO k = 1, nipf
    ...
    IF ( .NOT. exists ) THEN
      CALL message ( msg_warn, "EUMETSAT input file "//TRIM(ipfile(k))// &
                               " not found" )
      CYCLE
    ENDIF

!   3.1 EUMETSAT files should only have one record!
!   ------------------------------------------------

    CALL message ( msg_info, "Reading file "//TRIM(ipfile(k)) )

!   3.2 Read singlefiles (Loop over all profiles)
!   ---------------------------------------------
...
!-------------------------------------------------------------------------------
  ...
  SUBROUTINE Usage()
    PRINT *, 'Purpose:'
    PRINT *, '  Convert/thin a EUMETSAT netCDF file to a ROPP netCDF file'
    PRINT *, 'Usage:'
    PRINT *, '  > eum2ropp ip_file [ip_file...] [-o op_file] [-p thin_file|maxsamp]'
    PRINT *, '                     [-r resolution_group] [-m] [-a] [-u] [-i] [-d] [-h] [-v] [-l]'
    PRINT *, '   where ip_file(s) are EUMETSAT netCDF files'
    PRINT *, 'Options:'
    ...
    PRINT *, '  -h this help'
    PRINT *, '  -r resolution_group name of level 1B file, default thinned'
    PRINT *, '  -l get level 1a data into ROPP netCDF file'
    ...
    PRINT *, 'See eum2ropp(1) for details.'
    PRINT *, ''
