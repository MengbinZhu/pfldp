! $Id: bufr2ropp_ec.f90 2738 2011-03-22 15:34:18Z frdo $

PROGRAM bufr2ropp_ec

!****x* Programs/bufr2ropp_ec *
!
! NAME
...
! Fixed values
  ...
  INTEGER, PARAMETER    :: MaxLen = nb/4     ! enough for 500,000 octets
...
! Local variables
  ...
  INTEGER               :: iBUF(maxLen)      ! BUFR message
  INTEGER               :: LenBUF            ! Length of BUFR as read (bytes)

  CHARACTER (LEN=80)    :: cValues(nv)       ! Decoded character values
  REAL(dp)              :: Values(nv)        ! Decoded numeric   values (d.p.)
  CHARACTER (LEN=64)    :: Names(nd)         ! Element names
  CHARACTER (LEN=24)    :: Units(nd)         ! Element units
...
!--------------------------------------------------------------
    ...
    CALL PBOPEN ( BUFRunit,             &
                  ...
                  ierr )
    IF ( ierr /= 0 ) THEN
       CALL message ( msg_error, "Failed to open BUFR input file" )
      ExitErr = ErrIO
      CYCLE
    END IF
...
!--------------------------------------------------------------
    ...
    DO
      ...
      CALL PBBUFR ( BUFRunit,    &
                    iBUF,        &
                    MaxLen,      &
                    LenBUF,      &
                    ierr )

! Note that gfortran erroneously returns ierr=0 after all messages have been read, 
! and therefore goes into an infinite loop.  The following fix prevents this.
      IF ( LenBUF == 0 ) ierr = -1
      ...
      ELSE IF ( ierr == -3 ) THEN
        CALL message ( msg_error, "Message too long for read buffer" )
...
!--------------------------------------------------------------
      ...
      CALL BUFREX ( LenBUF, iBUF,                       &
                    Supp, Sec0, Sec1, Sec2, Sec3, Sec4, &
                    nd, Names,  Units,                  &
                    nv, Values, cValues,                &
                    ierr )

      IF ( ierr /= 0 ) THEN
        WRITE ( number, FMT=FmtNum ) ierr
        CALL message ( msg_error, "Error decoding BUFR message (code: "// &
                                  TRIM(ADJUSTL(number))//")" )
        CYCLE
      END IF

!--------------------------------------------------------------
! 3.4 Extract unexpanded & expanded descriptor lists
!     Ignore if this isn't an RO message (first unexpanded
!     descriptor must be same as ROdescr [030026])
!--------------------------------------------------------------

      CALL BUSEL ( nDescr, Descr, nExpDescr, ExpDescr, ierr )
      IF ( ierr /= 0 ) THEN
        WRITE ( number, FMT=FmtNum ) ierr
        CALL message ( msg_error, "Error expanding descriptors (code: "// &
                                  TRIM(ADJUSTL(number))//")" )
        CYCLE
      ELSE IF ( Descr(1) /= ROdescr ) THEN
...
!--------------------------------------------------------------
      ...
      nLev1a = 1
      nLev1b = ReplicationCount ( ExpDescr, Values, 005001 )
      nLev2a = ReplicationCount ( ExpDescr, Values, 007007 )
      nLev2b = ReplicationCount ( ExpDescr, Values, 007009 )
      IF ( nLev2b == 0 ) THEN
        nLev2c = 0
      ELSE
        nLev2c = 1
      END IF
      nLev2d = 0
      nFreq  = ReplicationCount ( ExpDescr, Values, 002121 )
...
SUBROUTINE version_info()
  ...
  PRINT *, 'bufr2ropp -  BUFR to ROPP netCDF decoder [ECMWF library]'
  PRINT *, ''
