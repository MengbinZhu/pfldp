! $Id: bufr2ropp_mod.f90 3551 2013-02-25 09:51:28Z idculv $
...
! Local parameters
  ...
  INTEGER            :: FOSsig   ! First-order statistics significance code
  INTEGER            :: TimeSig  ! Time signficance code
...
!-------------------------------------------------
  ...
  CALL ConvertCodes ( ROdata,        &
                      ...
                      Ccode,  Bcode, &
                      -1 )
  ProdType = NINT(Values(4))                    ! [002172] Product type
...
! Date/time of start of occultation
  ...
  TimeSig             = NINT(Values(6))         ! [008021] Time.sig (17=start)
...
! L1 data
      ...
      FOSsig = NINT(Values(IE+9))               ! [008023] First-order stats signif. (13=RMS)

      IF ( Values(IE+10) < MISSING ) &          ! [015037] B/angle error (rad)
           ROdata%Lev1b%BAngle_L1_Sigma(in) = Values(IE+10)

      FOSsig = NINT(Values(IE+11))              ! [008023] First-order stats signif. (missing=off)
...
! L2 data
      ...
      FOSsig = NINT(Values(IE+17))              ! [008023] First-order stats signif. (missing=off)
...
! Corrected bending angle (always encoded)
    ...
    FOSsig = NINT(Values(IE+21))                ! [008023] First-order stats signif. (13=RMS)

    IF ( Values(IE+22) < MISSING ) &            ! [015037] B/angle error (rad)
         ROdata%Lev1b%BAngle_Sigma(in) = Values(IE+22)

    FOSsig = NINT(Values(IE+23))                ! [008023] First-order stats signif. (missing=off)
...
! latitude- & height-dependent gravity model
    ...
    FOSsig = NINT(Values(IE+4))                 ! [008023] First-order stats signif. (13=RMS)

    IF ( Values(IE+5) < MISSING ) &             ! [015036] Refractivity error (N-units)
         ROdata%Lev2a%Refrac_Sigma(in) = Values(IE+5)

    FOSsig = NINT(Values(IE+6))                 ! [008023] First-order stats signif. (missing=off)
...
!-------------------------------------------------
  ...
  DO in = 1, ROdata%Lev2b%Npoints
    ...
    FOSsig = NINT(Values(IE+6))                 ! [008023] First-order stats signif. (13=RMS)
    ...
    FOSsig = NINT(Values(IE+10))                ! [008023] First-order stats signif. (missing=off)
