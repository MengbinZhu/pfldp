! $Id: ropp2bufr_mod.f90 3551 2013-02-25 09:51:28Z idculv $
...
!****
  ...
  PRINT *, '  -r  resolution group of netCDF4 EUM file'
  PRINT *, '  -d  outputs additonal diagnostics to stdout'
  PRINT *, '  -h  this help'
  PRINT *, '  -v  version information'
  PRINT *, 'Defaults:'
  PRINT *, '  Input  file name        : ropp.nc'
  ...
  PRINT *, '  Resolution              : ''thinned'' (only for EUM file)'
...
! Fixed parameters
  ...
  CHARACTER (LEN=*), PARAMETER :: Defdsn         = "ropp.nc" ! Default input file name
  ...
  DO WHILE ( ia <= narg )
    ...
    IF ( carg(1:1) == "-" ) THEN   ! is this an option introducer?
      ...
      SELECT CASE (carg(2:2))
        ...
        CASE ("r","R")             ! resolution group to use
          carg(1:2) = "  "
          IF ( carg(3:) == " " ) THEN
             ia = ia + 1
             CALL GETARG ( ia, carg )
          END IF
          resolution = ADJUSTL(carg)
