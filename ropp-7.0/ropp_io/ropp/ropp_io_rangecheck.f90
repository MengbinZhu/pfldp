! $Id: ropp_io_rangecheck.f90 3831 2013-08-27 15:22:48Z idculv $
!
...
! Local variables
  ...
  INTEGER  :: ikl  ! Keyword length
  INTEGER  :: ict  ! Invalid chr count
...
! The following strings are consist of a variable-length keyword
...
! if all characters are invalid, set whole string as unknown
  ...
  ROdata%Processing_centre = ADJUSTL(ROdata%Processing_centre)
  ikl = INDEX ( ROdata%Processing_centre, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Processing_centre(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Processing_Centre(i:i)) == 0 ) THEN
        ROdata%Processing_Centre(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Processing_Centre = "UNKNOWN"
  END IF

  ROdata%POD_Method = ADJUSTL(ROdata%POD_Method)
  ikl = INDEX ( ROdata%POD_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%POD_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%POD_Method(i:i)) == 0 ) THEN
        ROdata%POD_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%POD_Method = "UNKNOWN"
  END IF

  ROdata%Phase_Method = ADJUSTL(ROdata%Phase_Method)
  ikl = INDEX ( ROdata%Phase_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Phase_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Phase_Method(i:i)) == 0 ) THEN
        ROdata%Phase_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Phase_Method = "UNKNOWN"
  END IF

  ROdata%Bangle_Method = ADJUSTL(ROdata%Bangle_Method)
  ikl = INDEX ( ROdata%Bangle_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Bangle_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Bangle_Method(i:i)) == 0 ) THEN
        ROdata%Bangle_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Bangle_Method = "UNKNOWN"
  END IF

  ROdata%Refrac_Method = ADJUSTL(ROdata%Refrac_Method)
  ikl = INDEX ( ROdata%Refrac_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Refrac_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Refrac_Method(i:i)) == 0 ) THEN
        ROdata%Refrac_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Refrac_Method = "UNKNOWN"
  END IF

  ROdata%Meteo_Method = ADJUSTL(ROdata%Meteo_Method)
  ikl = INDEX ( ROdata%Meteo_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Meteo_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Meteo_Method(i:i)) == 0 ) THEN
        ROdata%Meteo_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Meteo_Method = "UNKNOWN"
  END IF

  ROdata%Thin_Method = ADJUSTL(ROdata%Thin_Method)
  ikl = INDEX ( ROdata%Thin_Method, " " ) - 1
  IF ( ikl > 0 ) THEN
    CALL To_Upper ( ROdata%Thin_Method(1:ikl) )
    ict = 0
    DO i = 1, ikl
      IF ( INDEX(validchars,ROdata%Thin_Method(i:i)) == 0 ) THEN
        ROdata%Thin_Method(i:i) = "_"
        ict = ict + 1
      END IF
    END DO
    IF ( ict == ikl ) ROdata%Thin_Method = "UNKNOWN"
  END IF
...
SUBROUTINE ropp_io_rangecheck_l1atype ( Lev1a ) ! (inout)
  ...
  USE ropp_utils,    ONLY : WHERE, ropp_MDFV, ropp_MDTV, ropp_ZERO
