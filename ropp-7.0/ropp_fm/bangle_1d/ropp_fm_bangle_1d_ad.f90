! $Id: ropp_fm_bangle_1d_ad.f90 3831 2013-08-27 15:22:48Z idculv $
...
!-------------------------------------------------------------------------------
  ...
  REAL(wp), DIMENSION(x%n_lev)   :: pdry_ad
  ...
  REAL(wp), DIMENSION(x%n_lev)     :: z_geop_ad          ! geopotential height of model levels
  REAL(wp), DIMENSION(x%n_lev)     :: zcomp_dry_inv, zcomp_wet_inv ! compressibilities
  REAL(wp), DIMENSION(x%n_lev)     :: zcomp_dry_inv_ad, zcomp_wet_inv_ad ! compressibilities
...
!-------------------------------------------------------------------------------
! 7. Adjoint of bending angle computation
!-------------------------------------------------------------------------------

  CALL ropp_fm_abel_ad(impact, refrac, y%impact, impact_ad, refrac_ad, y_ad)

!-------------------------------------------------------------------------------
! 8. Adjoint of impact parameter calculation
!-------------------------------------------------------------------------------
...
!----------------------------------------------
  ...
  x_ad%temp   = x_ad%temp - refrac_ad * (kap1*pdry*zcomp_dry_inv/(x%temp**2)     &
                                + 2.0_wp*kap2*pwvp*zcomp_wet_inv/(x%temp**3) &
                                  + kappa3*pwvp*zcomp_wet_inv/x%temp**2)

  refrac_ad = 0.0_wp

! 10 Adjoint of water vapor and dry air pressure calculation
!-----------------------------------------------------
