...
! calculate the water vapour and dry pressure
     ...
     zpwet_tl = zpwet/x%pres(i)*x_tl%pres(i) &
     & + x%pres(i)*epsilon_water/(epsilon_water + (1.0_wp - epsilon_water)*x%shum(i))**2*x_tl%shum(i)   
	     
! dry pressure

     zpd = x%pres(i) - zpwet

     zpd_tl = x_tl%pres(i) - zpwet_tl
...
! compressibility of water vapour
     ...
     zpot_tl = zpwet_tl/x%temp(i) - zpot/x%temp(i)*x_tl%temp(i)
...
! used to calculate the refractivity.  
    ...
    zcomp_dry_inv_tl(i) = -1.0_wp/zcomp2(i)**2*zcomp2_tl(i)
    
    zcomp_wet_inv(i) = 1.0_wp/zcomp3(i)
    
    zcomp_wet_inv_tl(i) = -1.0_wp/zcomp3(i)**2*zcomp3_tl(i)
