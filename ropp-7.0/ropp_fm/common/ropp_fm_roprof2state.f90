! $Id: ropp_fm_roprof2state.f90 3696 2013-06-17 08:48:37Z idculv $

SUBROUTINE ropp_fm_roprof2state1d(ro_data, x)

!****s* Copying/ropp_fm_roprof2state *
!
! NAME
...
! 7.2 Temperature sigmas
  ...
  DO i = 1, x%n_lev
    IF (ro_data%Lev2b%temp_sigma(i) > 0.0_wp) THEN
       x%cov%d(i + i*(i-1)/2) = ro_data%Lev2b%temp_sigma(i)**2
    ELSE
       x%cov_ok = .FALSE.
    END IF
  END DO
...
! 7.3 Humidity sigmas
  ...
  IF (x%use_logq) THEN
    DO i = 1, x%n_lev
       j = x%n_lev + i
       IF (ro_data%Lev2b%shum_sigma(i) > 0.0_wp) THEN
          x%cov%d(j + j*(j-1)/2) = (ro_data%Lev2b%shum_sigma(i) /   &
  ...
  ELSE
    DO i = 1, x%n_lev
       j = x%n_lev + i
       IF (ro_data%Lev2b%shum_sigma(i) > 0.0_wp) THEN
          x%cov%d(j + j*(j-1)/2) = ro_data%Lev2b%shum_sigma(i)**2
       ELSE
           x%cov_ok = .FALSE.
        ENDIF
    ENDDO
...
! 7.4 Surface pressure data

  IF (ro_data%Lev2c%press_sfc_sigma > 0.0_wp) THEN
    IF (x%use_logp) THEN
       x%cov%d(n + n*(n-1)/2) = (ro_data%Lev2c%press_sfc_sigma /    &
                                  ro_data%Lev2c%press_sfc)**2
    ELSE
       x%cov%d(n + n*(n-1)/2) = ro_data%Lev2c%press_sfc_sigma**2
    ENDIF
  ELSE
    x%cov_ok = .FALSE.
  ENDIF
...
! 9.2 Pressure sigmas
  ...
  IF (ro_data%Lev2c%press_sfc_sigma > 0.0_wp) THEN
    IF (x%use_logp) THEN
        x%cov%d(1) = (ro_data%Lev2c%press_sfc_sigma/ro_data%Lev2c%press_sfc)**2
    ELSE
        x%cov%d(1) = ro_data%Lev2c%press_sfc_sigma**2
    ENDIF
  ELSE
    x%cov_ok = .FALSE.
  ENDIF
  ...
  IF (x%use_logp) THEN
    DO i = 2, x%n_lev+1
       IF (ro_data%Lev2b%press_sigma(i-1) > 0.0_wp) THEN
          x%cov%d(i + i*(i-1)/2) = (ro_data%Lev2b%press_sigma(i-1) /    &
                                     ro_data%Lev2b%press(i-1))**2
       ELSE
          x%cov_ok = .FALSE.
       ENDIF
    ENDDO
  ELSE
    DO i = 2, x%n_lev+1
       IF (ro_data%Lev2b%press_sigma(i-1) > 0.0_wp) THEN
          x%cov%d(i + i*(i-1)/2) = ro_data%Lev2b%press_sigma(i-1)**2
       ELSE
          x%cov_ok = .FALSE.
       ENDIF
    ENDDO
...
!****s* Copying/ropp_fm_roprof2state_2d *
!
! NAME
...
!****s* Copying/ropp_fm_roprof2d2state1d *
!
! NAME
