...
! adjoint bit
  ...
  DO i = x%n_lev, 1, -1
    ...
    zcomp3_ad(i) = zcomp3_ad(i) -1.0_wp/zcomp3(i)**2*zcomp_wet_inv_ad(i)
  
    zcomp_wet_inv_ad(i) = 0.0_wp
  
    zcomp2_ad(i) = zcomp2_ad(i) - 1.0_wp/zcomp2(i)**2*zcomp_dry_inv_ad(i)
  
    zcomp_dry_inv_ad(i) = 0.0_wp
...
! compressibility of water vapour
     ...
     zpwet_ad = zpwet_ad + zpot_ad/x%temp(i)
     
     x_ad%temp(i) =  x_ad%temp(i) - zpot/x%temp(i)*zpot_ad
...
!!     zpot_tl = zpot/x%pres(i)*x_tl%pres(i) - zpot/x%temp(i)*x_tl%temp(i)
  
!!     zx = zpwet/x%pres(i)
     
!!     zx_tl = 1.0_wp/x%pres(i)*zpwet_tl - zx/x%pres(i)*x_tl%pres(i) 
     
     zpwet_ad = zpwet_ad + 1.0_wp/x%pres(i)*zx_ad 
     x_ad%pres(i) = x_ad%pres(i) - zx/x%pres(i)*zx_ad
     zx_ad = 0.0       
   
     x_ad%pres(i) = x_ad%pres(i) + zpot/x%pres(i)*zpot_ad 
     x_ad%temp(i) = x_ad%temp(i) - zpot/x%temp(i)*zpot_ad
     zpot_ad = 0.0_wp
...
!!   zpd_prime = PRES_PRIME(i) - zpwet_prime 
   ...
   x_ad%pres(i) = x_ad%pres(i) + zpd_ad
...
!!             & + PRES(i)*z_eps/(z_eps + (1.0_wp - z_eps)*PQ(i))**2*PQ_PRIME(i)   
   ...
   x_ad%pres(i) = x_ad%pres(i) + zpwet/x%pres(i)*zpwet_ad
   x_ad%shum(i) = x_ad%shum(i) +  &
   x%pres(i)*epsilon_water/( epsilon_water+ (1.0_wp - epsilon_water)*x%shum(i))**2*zpwet_ad
   zpwet_ad = 0.0_wp
   
   
!!   ztc_prime = PTEMP_PRIME(i)
     
   x_ad%temp(i) = x_ad%temp(i) + ztc_ad
   ztc_ad = 0.0_wp
