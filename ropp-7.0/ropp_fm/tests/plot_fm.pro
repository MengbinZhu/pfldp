PRO plot_fm, file, extra_command=extra_command
...
plot, ropp_bangle[*,0], (ropp_impact[*,0]-min(ropp_impact[*,0]))/1000.0, ystyle=1, yrange=[0.1,100.],$
	ytitle='Impact Height [km]', xtitle='FM bending angle [rad]', charsize=1.6, /ylog
for isce=1,nsce-1 do $
  oplot, ropp_bangle[*,isce], (ropp_impact[*,isce]-min(ropp_impact[*,isce]))/1000.0, linestyle=isce
...
pinfo = [ ['0.05','0.10','ROPP forward modelled refractivity and bending angle using ECMWF background profiles'], $
	  ...
	  ['0.05','0.01','Calls ropp_fm_bgro_1d'+extra_command+'. Produced on: '+systime() ] ]
