...
! ----------------
  ...
  real(wp), dimension(size(r_leo,2))   :: occ_cart
...
! --------------------------------------------------------------
  ...
  ! 1.6.1 Find occultation point cartesian coordinates (at surface)
  call geod2cart(lat, lon, 0.0_wp, occ_cart)

  ! 1.6.2 Cross-section azimuth

  theta = vector_angle(vector_product(r_gns(iocc,:),r_leo(iocc,:)), &
                        vector_product(pa, occ_cart))
                        
  if (Dot_Product(r_gns(iocc,:)-r_leo(iocc,:),vector_product(pa, perigee(iocc,:))) < 0) then
    theta=2.0*Pi-theta
  endif

  azimuth = theta * rad2deg

  ! 1.6.3 Compute curvature
